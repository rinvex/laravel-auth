...
namespace Rinvex\Fort\Test\Guards;
...
class SessionGuardTest extends TestCase
{
	...
	protected function getGuard()
	{
		...
		return new \Rinvex\Fort\Guards\SessionGuard('default', $provider, $session, $request);
	}
	...
	{
		list($session, $provider, $request, $cookie) = $this->getMocks();
		$guard = m::mock('Rinvex\Fort\Guards\SessionGuard[check,attempt]', ['default', $provider, $session]);
	...
	{
		list($session, $provider, $request, $cookie) = $this->getMocks();
		$guard = m::mock('Rinvex\Fort\Guards\SessionGuard[check]', ['default', $provider, $session]);
	...
	// {
	// 	list($session, $provider, $request, $cookie) = $this->getMocks();
	// 	$guard = $this->getMockBuilder('Rinvex\Fort\Guards\SessionGuard')->setMethods(['login'])->setConstructorArgs(['default', $provider, $session, $request])->getMock();
	// 	$guard->setDispatcher($events = m::mock('Illuminate\Contracts\Events\Dispatcher'));
	// 	$events->shouldReceive('dispatch')->once()->with(m::type(Attempting::class));
	...
	{
		list($session, $provider, $request, $cookie) = $this->getMocks();
		$mock = $this->getMockBuilder('\Rinvex\Fort\Guards\SessionGuard')->setMethods(['getName'])->setConstructorArgs(['default', $provider, $session, $request])->getMock();
	...
	*/
	...
	{
		list($session, $provider, $request, $cookie) = $this->getMocks();
		$mock = $this->getMockBuilder('Rinvex\Fort\Guards\SessionGuard')->setMethods(['user'])->setConstructorArgs(['default', $provider, $session, $request])->getMock();
	...
	{
		list($session, $provider, $request, $cookie) = $this->getMocks();
		$mock = $this->getMockBuilder('Rinvex\Fort\Guards\SessionGuard')->setMethods(['getName', 'getRecallerName', 'recaller'])->setConstructorArgs(['default', $provider, $session, $request])->getMock();
	...
	{
		list($session, $provider, $request, $cookie) = $this->getMocks();
		$mock = $this->getMockBuilder('Rinvex\Fort\Guards\SessionGuard')->setMethods(['getName', 'recaller'])->setConstructorArgs(['default', $provider, $session, $request])->getMock();
		$mock->setCookieJar($cookies = m::mock('Illuminate\Cookie\CookieJar'));
		$user = m::mock('Illuminate\Contracts\Auth\Authenticatable');
		$user->shouldReceive('setRememberToken')->once();
		$mock->expects($this->once())->method('getName')->will($this->returnValue('foo'));
	...
	{
		list($session, $provider, $request, $cookie) = $this->getMocks();
		$mock = $this->getMockBuilder('Rinvex\Fort\Guards\SessionGuard')->setMethods(['clearUserDataFromStorage'])->setConstructorArgs(['default', $provider, $session, $request])->getMock();
	...
	{
		list($session, $provider, $request, $cookie) = $this->getMocks();
		$guard = new \Rinvex\Fort\Guards\SessionGuard('default', $provider, $session, $request);
	...
	public function testLoginUsingIdLogsInWithUser()
	{
		...
		$guard = m::mock('Rinvex\Fort\Guards\SessionGuard', ['default', $provider, $session])->makePartial();
