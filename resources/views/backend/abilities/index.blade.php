...
{{-- Page Title --}}
@section('title')
    {{ config('app.name') }} » {{ trans('rinvex/fort::backend/forms.heading.abilities') }}
@stop
    ...
    <div class="container">
        ...
        <section class="panel panel-default">
            ...
            <header class="panel-heading">
                <h4>
                    {{ trans('rinvex/fort::backend/forms.heading.abilities') }}
                                ...
                                <tr>
                                    ...
                                    <td class="small">
                                        @if($ability->created_at)
                                            <div>
                                                {{ trans('rinvex/fort::backend/forms.common.created_at') }}: <time datetime="{{ $ability->created_at }}">{{ $ability->created_at->format('Y-m-d') }}</time>
                                            </div>
                                        @endif
                                        @if($ability->updated_at)
                                            <div>
                                                {{ trans('rinvex/fort::backend/forms.common.updated_at') }}: <time datetime="{{ $ability->updated_at }}">{{ $ability->updated_at->format('Y-m-d') }}</time>
                                            </div>
                                        @endif
                                    </td>
